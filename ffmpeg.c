...
static void dump_packet_info(const char *codectype, AVPacket *pkt, char *pts, char *dts) {
	static FILE *fp3;
	unsigned char hash[20] = {}, s7[128];
	char hdr[1024];
	uint32_t hdrlen, datalen;

	SHA1_CTX ctx = {};
	SHA1Init(&ctx);
	SHA1Update(&ctx, pkt->data, pkt->size);
	SHA1Final(hash, &ctx);

	for (int i = 0; i < 20; i++)
		sprintf(s7+i*2, "%02x", hash[i]);

	if (fp3 == NULL)
		fp3 = fdopen(3, "w");

	sprintf(hdr, "%s,%d,%.8s,%s,%s,%d", codectype, pkt->size, s7, pts, dts, !!(pkt->flags&AV_PKT_FLAG_KEY));

	hdrlen = (uint32_t)strlen(hdr);
	fwrite(&hdrlen, 1, 4, fp3);
	fwrite(hdr, 1, hdrlen, fp3);

	datalen = (uint32_t)pkt->size;
	fwrite(&datalen, 1, 4, fp3);
	fwrite(pkt->data, 1, datalen, fp3);
}

>>>>>>> e3befc6... modify ffmpeg.c
static void write_frame(AVFormatContext *s, AVPacket *pkt, OutputStream *ost)
{
    ...
    pkt->stream_index = ost->index;
		...
		if (debug_pkt) {
			dump_packet_info(
				av_get_media_type_string(ost->enc_ctx->codec_type), pkt,
